...
def find_header_row(input_excel: str, max_rows: int = 15) -> int:
    """
    Detect the header row dynamically by searching for required columns.
    ...
    for idx, row in preview.iterrows():
        ...
        if REQUIRED_COLUMNS.issubset(values):
            logger.info(f"Header found at Excel row {idx + 1}")
            return idx

    raise ValueError(
        "Could not detect header row. Required columns not found."
    )
...
def process_excel(input_excel: str, output_dir: str):
    try:
        ...
        header_row = find_header_row(input_excel)
        ...
        logger.info(
            f"Excel loaded successfully using header row {header_row + 1}. "
            f"Rows: {len(df)}"
        )
